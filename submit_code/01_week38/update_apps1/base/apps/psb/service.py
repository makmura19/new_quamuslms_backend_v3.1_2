...
class MainService(BaseService):
    ...
    # def create__role(role):
    #     roles = {
    #         "ppdb_adm": Role.PPDB_ADM,
    #         "ppdb_validator": Role.PPDB_VALIDATOR,
    #         "ppdb_cashier": Role.PPDB_CASHIER,
    #         "ppdb_checker": Role.PPDB_CHECKER,
    #         "ppdb_unit_coordinator": Role.PPDB_UNIT_COORDINATOR,
    #         "ppdb_unit_staff": Role.PPDB_UNIT_STAFF,
    #         "ppdb_finance": Role.ppdb_finance,
    #         "ppdb_dormitory_staff": Role.PPDB_DORMITORY_STAFF,
    #         "ppdb_medic": Role.PPDB_MEDIC
    #     }
    ...
    def validate_update(value, _extra, secret, user, old_data=None):
        ...
        if value.get("holding_id"):
            if ObjectId(_extra.get("finance_va_config",{}).get("holding_id")) != ObjectId(value.get("holding_id")):
                raise ValidationError("Data yayasan dan config VA tidak sesuai.")
        elif value.get("school_id"):
            if value.get("school_id") not in _extra.get("finance_va_config",{}).get("school_ids"):
                raise ValidationError("Data sekolah dan config VA tidak sesuai.")
