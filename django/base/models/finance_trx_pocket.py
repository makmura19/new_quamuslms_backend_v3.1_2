...
class FinanceTrxPocketSchema(Schema):
...
class FinanceTrxPocket(BaseModel):
    type_id = DictUtil.get_id_type_from_dataclass(FinanceTrxPocketData)
    collection_name = "finance_trx_pocket"
    schema = FinanceTrxPocketSchema
    search = ["student_nis", "student_name", "name"]
    object_class = FinanceTrxPocketData
    foreign_key = {
        "school": {
            "local": "school_id",
            "foreign": "_id",
