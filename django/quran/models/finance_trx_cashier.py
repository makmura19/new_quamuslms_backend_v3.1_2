...
class FinanceTrxCashierSchema(Schema):
    ...
    staff_id = ObjectIdField(required=True, allow_none=False)
...
class FinanceTrxCashier(BaseModel):
    type_id = DictUtil.get_id_type_from_dataclass(FinanceTrxCashierData)
    collection_name = "finance_trx_cashier"
    schema = FinanceTrxCashierSchema
    search = ["student_nis", "student_name", "name"]
    object_class = FinanceTrxCashierData
    foreign_key = {
        "holding": {
            "local": "holding_id",
            "foreign": "_id",
